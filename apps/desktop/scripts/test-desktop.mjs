...
// electron-builder names the executable after build.executableName (falling
// back to productName) and installers after build.artifactName, whose prefix
// here is the product name. Read both from package.json so a rename never
// leaves this script looking for a binary that no longer exists.
const EXECUTABLE = PACKAGE_JSON.build?.executableName || PACKAGE_JSON.productName || PACKAGE_JSON.name
const ARTIFACT_PREFIX = PACKAGE_JSON.productName || EXECUTABLE
...
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, `${EXECUTABLE}.app`)
    return {
      appPath,
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, `${EXECUTABLE}.exe`),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, EXECUTABLE),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function validateBundle() {
  if (!exists(APP.binary)) {
    // Name what electron-builder did produce, so a renamed executable is
    // diagnosed from the log instead of guessed at.
    const produced = exists(APP.appPath) ? fs.readdirSync(APP.appPath).join(', ') : '(app directory missing)'
    die(`Missing packaged app binary: ${APP.binary}\nContents of ${APP.appPath}: ${produced}`)
  }
