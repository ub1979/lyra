...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Lyra is ready', message: `${provider} connected.` })
}
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ...
    : 'Connected, but Lyra still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Lyra could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
