...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'AppIT is ready', message: `${provider} connected.` })
}
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ...
    : 'Connected, but AppIT still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'AppIT could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
