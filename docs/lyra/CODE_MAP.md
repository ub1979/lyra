...
| Local native-dialog recovery | `tools/browser_dialog_support.py`, `browser_tool.py`, `browser_supervisor.py`, `browser_dialog_tool.py` | Real default-backend accept/cancel/prompt, same-browser continuation, sidecar routing and existing CDP/security suites; [change record](changes/2026-09-19-notes-trial-reliability-repairs.md) |
...
| Validated dependency waits | `hermes_cli/kanban_dependencies.py`, `kanban_db.block_task` | Real SQLite and tool tests reject nonexistent/reversed waits; preserve valid resume and stale-run fencing. See [QA routing repairs](changes/2026-09-19-qa-dependency-routing.md) |
| QA worker role boundary | `plugins/ultimate-builder/qa_worker_policy.py`, shared `qa-evidence` skill | Existing pre-tool hook rejects QA scheduling and application file edits; tests use real plugin registration, task storage and dispatch. Terminal tests remain available; not an OS sandbox. |
| Project Stop / isolated descendant scope | `plugins/ultimate-builder/project_task_scope.py`, `project_run_stop.py`, `hermes_cli/kanban_archive.py` | Creation provenance plus current links protect unrelated work. Per-board atomic archive precedes worker termination; real claim-race, process, rollback, cross-project/board and relocation tests. |
