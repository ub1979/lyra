...
| Coordinator context budget | `tui_gateway/studio_budget.py`, `tools/budget_config.py` (`inline_caps`), `agent/tool_executor.py` | Budget composition, marked-agent routing, pinned inline truncation and prune-policy tests |
...
| Reproducible file index | `scripts/lyra_code_map.py` | Tracked-files-only inventory test on a temporary repository; `--check` in CI |
