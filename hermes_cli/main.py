...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    """No-op: Lyra does not track the upstream Hermes repository.

    This used to offer — defaulting to *yes* on a bare Enter — to add
    ``NousResearch/hermes-agent`` as an ``upstream`` remote and pull its main
    branch into this checkout whenever origin was behind it. Lyra has diverged
    far past the point where that is safe: its own agents, playbooks, dashboard,
    skills library and API surface would be merged against a codebase that knows
    nothing about them, during a routine ``hermes update``.

    Updates now come from this repository's own ``origin`` and nowhere else.
    Pulling a specific upstream fix is a deliberate act — add the remote by hand,
    cherry-pick it, and write a change record — not something an update command
    does on its own.
    """
    return
