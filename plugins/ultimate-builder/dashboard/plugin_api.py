...
def _project_runs_module():
    return _load_project_module("project_runs")
...
def _merge_project_run_state(ledger, run_state, project=None):
    return _load_project_module("project_progress")._merge_project_run_state(ledger, run_state, project)


@lru_cache(maxsize=4)
def _load_project_module(name):
    """Share sibling loading without depending on the plugin's install name."""
    path = Path(__file__).resolve().parents[1] / f"{name}.py"
    spec = importlib.util.spec_from_file_location(f"lyra_{name}", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load project feature: {name}")
...
def _relocate_project_metadata(
    ...
    warnings: list[str] = []
    try:
        CheckpointManager(enabled=True).relocate_project(str(source), str(destination))
    except (OSError, ValueError):
        warnings.append("Recovery history could not be attached to the new folder. Its original copy is still saved.")
    try:
