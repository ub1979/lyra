...
2. Present it with `project_run(action="preview")` and ask the returned question with `clarify` exactly as given (Approve / Change / Skip). Development cannot be queued until the user answers; on Change, regenerate and present again.
...
| Rule | Details |
|------|---------|
| **Queue by registered phase id** | Queue durable specialist work through `hermes project-run queue` using the registered phase ids. Use `delegate_task` only for a short disposable lookup when it is available. Requirements work stays in the main conversation. |
| **Model selection follows confirmed routing** | Use the provider/model pair confirmed in Studio for every override. Omit both values to inherit the project model. Reserve the strongest available models for judgment-heavy roles and use faster models for bounded mechanical work. |
