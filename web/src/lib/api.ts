...
// injects ``window.__IDRAK_IT_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__IDRAK_IT_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (runtime maintenance and gateway restart actions). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__IDRAK_IT_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__IDRAK_IT_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__IDRAK_IT_SESSION_TOKEN__`` is valid, so the next
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__IDRAK_IT_AUTH_REQUIRED__) {
 ...
 * ``window.__IDRAK_IT_SESSION_TOKEN__`` directly, so the gated-mode ticket
