...
export const GUIDED_OPENING_MESSAGE = "whats the great idea you wanna build";

/** New empty projects wait for the person; older launch links retain their behavior. */
export function builderStartsOnOpen(seed: string): boolean {
  try {
    const match = seed.match(/IDRAK_INTERNAL_SETUP_BEGIN\s+(.+?)\s+IDRAK_INTERNAL_SETUP_END/);
    return !match || JSON.parse(match[1]).start_on_open !== false;
  } catch { return false; }
}
...
export function guidedSetupSeed(
  ...
  return `IDRAK_INTERNAL_SETUP_BEGIN ${JSON.stringify({
    ...
    team_selection_gate: guidedTeamSelectionDirective(teamSelectionMode),
    execution_gate: guidedProjectExecutionTurnDirective(specialists),
    preview_gate: "Before asking for visual approval, call project_run action=preview. For alternative designs, supply preview_options mapping their names to files under .sdlc/preview. Present exactly the returned question and choices through clarify; do not collect a separate informal approval first. Pass the selected build_profile to every queued phase, including Documentation. After dispatch, do not edit worker-owned artifacts or use cron to execute checks; report progress and return to the user.",
    workspace,
