...
function version(overrides: Partial<LyraVersion> = {}): LyraVersion {
  return {
    channel: "beta",
    display: "beta v0.19.50",
    notes: ["Researcher is now selectable."],
    release_name: "the reply keeps its bubble",
    released: "2026-09-16",
    title: "the reply keeps its bubble",
...
describe("versionLabel", () => {
  it("shows Lyra's own version", () => {
    expect(versionLabel(version())).toBe("beta v0.19.50");
  });
...
describe("versionTooltip", () => {
  it("names the release and says it is current", () => {
    ...
    expect(text).toContain(
      "beta v0.19.50 — the reply keeps its bubble",
    );
