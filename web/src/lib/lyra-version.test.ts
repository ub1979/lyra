...
function version(overrides: Partial<LyraVersion> = {}): LyraVersion {
  return {
    channel: "beta",
    display: "beta v0.19.9",
    notes: ["Researcher is now selectable."],
    release_name: "truthful project progress",
    released: "2026-08-29",
    title: "truthful project progress",
...
describe("versionLabel", () => {
  it("shows Lyra's own version", () => {
    expect(versionLabel(version())).toBe("beta v0.19.9");
  });
...
describe("versionTooltip", () => {
  it("names the release and says it is current", () => {
    ...
    expect(text).toContain("beta v0.19.9 — truthful project progress");
