...
| `HERMES_API_CALL_STALE_TIMEOUT` | Non-streaming stale-call timeout in seconds (default: `90`). Local providers use the finite local-provider ceiling below when this is left unset; cloud requests may scale upward for very large contexts. Also configurable via `providers.<id>.stale_timeout_seconds` or `providers.<id>.models.<model>.stale_timeout_seconds` in `config.yaml`. |
...
| `HERMES_STREAM_STALE_TIMEOUT` | Stale stream detection timeout in seconds (default: `180`). Local providers use the finite local-provider ceiling below when this is left unset. Triggers connection kill if no chunks arrive within this window. |
| `HERMES_LOCAL_STREAM_STALE_TIMEOUT` | Stale-call ceiling for local providers (Ollama, oMLX, llama-cpp) in seconds (default: `900`). It applies to streaming and non-streaming calls when their base stale timeout is implicit, preserving slow local prompt prefill while ensuring a wedged server eventually recovers. Also configurable via the backwards-compatible `agent.local_stream_stale_timeout` key in `config.yaml`. |
